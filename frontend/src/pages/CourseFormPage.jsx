...
export default function CourseFormPage() {
  ...
  useEffect(() => {
    if (isEdit) {
      setLoading(true)
      // Load by ID via admin list since we have id
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex gap-4">
            <button type="submit" disabled={saving} className="btn-primary flex items-center gap-2 disabled:opacity-70">
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" />Saving...</> : <><Save className="w-4 h-4" />{isEdit ? 'Update Course' : 'Create Course'}</>}
            </button>
